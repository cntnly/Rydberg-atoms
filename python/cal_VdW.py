...
mask3=((np.tril(np.ones_like(V_VdW))!=0)&(np.abs(Pair1_l1 -Pair2_l1)==1)&(np.abs(Pair1_l2 -Pair2_l2)==1))
...
#mask2= ((np.abs(Pair1_l1 -Pair2_l1)==1)&(np.abs(Pair1_l2 -Pair2_l2)==1))
V_A[mask3] = A_vec(Pair1_l1[mask3], Pair1_m1[mask3], Pair1_l2[mask3], Pair1_m2[mask3], Pair2_l1[mask3], Pair2_m1[mask3], Pair2_l2[mask3], Pair2_m2[mask3], theta)
...
#mask2 = (V_A!=0)
V_R[V_A!=0] = rad_vec(Pair1_n1[mask2], Pair1_l1[mask2], Pair2_n1[mask2], Pair2_l1[mask2], 1)*rad_vec(Pair1_n2[mask2], Pair1_l2[mask2], Pair2_n2[mask2], Pair2_l2[mask2], 1)
...
if __name__ == '__main__':             
    ...
    imshow(V_VdW)
    colorbar()
mask1 = ((np.tril(np.ones_like(V_VdW))!=0)&(np.abs(Pair1_l1 -Pair2_l1)==1))
V_R = np.zeros_like(V_VdW)
V_A = np.zeros_like(V_VdW)
V_A[mask1] = A_Stark_vec(Pair1_l1[mask1], Pair1_m1[mask1], Pair2_l1[mask1], Pair2_m1[mask1])*(Pair1_n2[mask1] == Pair2_n2[mask1]) *(Pair1_l2[mask1] == Pair2_l2[mask1])*(Pair1_m2[mask1] == Pair2_m2[mask1])
mask2 = (V_A !=0)
if len(V_A[V_A!=0])!=0:
    V_R[V_A !=0] = rad_vec(Pair1_n1[mask2], Pair1_l1[mask2], Pair2_n1[mask2], Pair2_l1[mask2], 1)
    V_Stark1[V_A !=0] = V_R[V_A!=0]*V_A[V_A!=0]
    V_Stark1 = V_Stark1 + V_Stark1.T - np.diag(V_Stark1.diagonal())
    V_Stark1 = V_Stark1*1e-9

mask1 = ((np.tril(np.ones_like(V_VdW))!=0)&(np.abs(Pair1_l2 -Pair2_l2)==1))
V_R = np.zeros_like(V_VdW)
V_A = np.zeros_like(V_VdW)
V_A[mask1] = A_Stark_vec(Pair1_l2[mask1], Pair1_m2[mask1], Pair2_l2[mask1], Pair2_m2[mask1])*(Pair1_n1[mask1] == Pair2_n1[mask1]) *(Pair1_l1[mask1] == Pair2_l1[mask1])*(Pair1_m1[mask1] == Pair2_m1[mask1])
mask2 = (V_A !=0)
if len(V_A[V_A!=0])!=0:
    V_R[V_A !=0] = rad_vec(Pair1_n2[mask2], Pair1_l2[mask2], Pair2_n2[mask2], Pair2_l2[mask2], 1)
...
mask1=(np.tril(np.ones_like(V_VdW))!=0)
V_A[mask1] = A_Zeeman_vec(Pair1_l1[mask1], Pair1_m1[mask1], Pair2_l1[mask1], Pair2_m1[mask1], Bx, By, Bz)*(Pair1_n1[mask1] == Pair2_n1[mask1])*(Pair1_n2[mask1] == Pair2_n2[mask1]) *(Pair1_l2[mask1] == Pair2_l2[mask1])*(Pair1_m2[mask1] == Pair2_m2[mask1])
V_Zeeman += V_A


V_A = 0j*np.zeros_like(V_VdW)
V_A[mask1] = A_Zeeman_vec(Pair1_l2[mask1], Pair1_m2[mask1], Pair2_l2[mask1], Pair2_m2[mask1], Bx, By, Bz)*(Pair1_n2[mask1] == Pair2_n2[mask1]) *(Pair1_n1[mask1] == Pair2_n1[mask1]) *(Pair1_l1[mask1] == Pair2_l1[mask1])*(Pair1_m1[mask1] == Pair2_m1[mask1])
V_Zeeman += V_A
...
for i,elm in enumerate(R):
    #out_egr[i] = LAeigvalsh(EI + V_VdW* coef/(elm**3)) - pair_12.E_Zeeman
